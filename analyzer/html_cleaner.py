...
def clean_all_pages(pages_dir: str, output_dir: str) -> int:
    ...
    for i, html_file in enumerate(html_files, 1):
        ...
        slug = rel.parent.name if rel.name == "index.html" else rel.stem
        slug = re.sub(r"^app-\d+-", "", slug)
        out = output_path / f"{slug}.html"
...
def clean_application(app_name: str) -> dict:
    ...
    count = clean_all_pages(str(stitched_dir), str(cleaned_dir))
    return {"pages_cleaned": count, "cleaned_dir": str(cleaned_dir)}
