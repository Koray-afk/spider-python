...
def stitch_pages(pages_dir: str = "pages") -> int:
    ...
    if not sitemap:
        ...
        if sitemap:
            ...
            print(f"  rebuilt {sitemap_path} from {len(sitemap)} index.meta files")
    ...
    for html_file in html_files:
        ...
        html_file.write_text(
            finalize_html(html, slug_route_map, from_slug, layout),
            encoding="utf-8",
        )
        count += 1
        print(f"  stitched {from_slug}/index.html")

    entry = pick_entry_slug(sitemap)
    print(f"  layout: {layout} ({count} pages)")
    if entry:
        print(f"  entry:  pages/{entry}/index.html")
    return count


if __name__ == "__main__":
    print("[*] Stitching sidebar nav across crawled pages...")
    total = stitch_pages()
    print(f"\n✅ Done — {total} pages wired together.")
    print("   Serve:  python3 -m http.server 8080  (required — do not open file:// directly)")
    print("   Open:   http://localhost:8080/pages/<dashboard-slug>/index.html")
